...
const DataManagerProvider = ({ children }: DataManagerProviderProps) => {
  ...
  const componentsThatHaveOtherComponentInTheirAttributes = useMemo(() => {
  ...
  const allComponentsCategories = useMemo(() => {
    return retrieveSpecificInfoFromComponents(components, ['category']);
  }, [components]);

  const componentsGroupedByCategory = useMemo(() => {
    return groupBy(components, 'category');
  }, [components]);

  const sortedContentTypesList = useMemo(() => {
